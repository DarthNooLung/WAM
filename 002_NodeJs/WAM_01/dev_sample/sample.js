...
//EditZoneStart ----------------------------------------------------------------
NetFunnel.TS_HOST = "netfunnel.hdec.co.kr"; // Default TS host
NetFunnel.TS_PORT = 80; 				// Default TS port
NetFunnel.TS_PROTO = 'http'; 			// Default TS protocol [http|https]
NetFunnel.TS_QUERY = "ts.wseq"; 		// Default request query
NetFunnel.TS_SERVICE_ID = "service_1"; 			// Default TS Service id
NetFunnel.TS_ACTION_ID = "EHRD_01"; 			// Default TS Action id
NetFunnel.TS_MAX_TTL = 30; 				// Default max ttl (second) 5~30
NetFunnel.TS_CONN_TIMEOUT = 3; 				// Default connect timeout (second)
NetFunnel.TS_CONN_RETRY = 2; 				// Default connect retry count
NetFunnel.TS_COOKIE_ID = "Hdec_NetFunnel_ID"; 	// Default Cookie ID
NetFunnel.TS_COOKIE_TIME = 10; 				// Default Cookie Time (minute)
NetFunnel.TS_COOKIE_DOMAIN = "*.hdec.co.kr"; 				// Default Cookie Domain
...
NetFunnel.Util = {
    ...
    isSmartPhone: function () {
        var b = new Array("iPhone", "iPod", "iPad", "BlakBerry", "Android", "WindowsCE", "LG", "MOT", "SAMSUNG", "SonyEricsson", "Nokia", "Webos", "Opera mini", "Opera mobi", "Iemobile");
        try {
...
NetFunnel.writeDebugMsg = function (k, j, c) {
    ...
    k.document.write("<div onload='this.focus()' style='width:650;overflow:hidden;padding:1px;border:1px solid #eeeeee;margin:0px;font-size:10px;font-family:monospace;background-color:" + b + h + "'>" + n + c.substring(0, 50) + "</div>")
};
    ...
    if (NetFunnel.gLastData && NetFunnel.gLastData.time_left >= 0) {
        if (NetFunnel.gPop) {
            ...
            if (this._gNWaitView != 0) {
                if (parseInt(this._gNWaitView) < parseInt(NetFunnel.gLastData.nwait)) {
...
NetFunnel.Popup = function (f, j, d, b, o, h) {
    ...
    if (!c || NetFunnel.SkinUtil.prevID != NetFunnel.gSkinId) {
        ...
        if (l) {
            ...
            var a = new NetFunnel.ProgressBar(l, {
                width: (k),
                count: 50,
                interval: 50
            }, this._mDoc);
...
NetFunnel.TsClient.prototype.aliveNoticeProc = function (d, g, a, f) {
    ...
    if (g && g != "" && a && a != "" && !this.mConfig.config_use) {
        this.ip = (this.mConfig.host == g) ? g : this.mConfig.host;
        this.port = (this.mConfig.port == a) ? a : this.mConfig.port;
...
NetFunnel.TsClient.prototype.setComplete = function (h, f, d, e) {
    ...
    if (f && f != "" && d && d != "" && !this.mConfig.config_use) {
        this.ip = (this.mConfig.host == f) ? f : this.mConfig.host;
        this.port = (this.mConfig.port == d) ? d : this.mConfig.port;
